...
export default function FormerCAGCards() {
  ...
  const [cags, setCags] = useState<FormerCAG[]>([]);
  ...
  useEffect(() => {
    fetch('/api/former-cags')
      .then(res => res.json())
      .then(data => {
        if (Array.isArray(data) && data.length > 0) {
          const mapped = data.map((x: any) => ({
            id: String(x.id),
            name: isHindi ? (x.name_hi || x.name_en) : x.name_en,
            tenure: `(${x.tenure_from} - ${x.tenure_to})`,
            image: x.image_url || undefined
          }));
          setCags(mapped);
        } else {
          setCags(isHindi ? legacyListHindi : legacyListEnglish);
        }
      })
      .catch(() => setCags(isHindi ? legacyListHindi : legacyListEnglish));
  }, [isHindi]);
          ...
          <div className="former-cag-card__content" data-name="Frame 1000005458">
            {cag.image ? (
              <img 
                src={cag.image} 
                alt={cag.name} 
                className="former-cag-card__photo w-full object-cover rounded-t-xl" 
                style={{ height: '180px' }}
              />
            ) : (
