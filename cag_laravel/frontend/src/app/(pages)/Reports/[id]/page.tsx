...
export default function ReportDetailPage({ params }: SubpageProps) {
  ...
  React.useEffect(() => {
    // Initial static fallback check
    const localFallback = dataManager.getReports().find(r => r.id === resolvedParams.id) || {
      ...
      file_url: '#',
      description: 'This audit report presents findings from our field investigations, compliance checks, and transaction testing. We examined administrative systems, procurement records, and project schedules to verify whether expenditures align with budgetary approvals.'
    };
    setReportDetails(localFallback);

    // Fetch live single report details from DB API
    fetch(`/api/reports/${resolvedParams.id}`)
      .then(res => res.json())
      .then(data => {
        if (data && !data.error) {
      ...
      .catch(err => console.warn('Single report details fetch failed, using fallback:', err));
  }, [resolvedParams.id]);

  if (!reportDetails) return <div className="p-10 text-center text-zinc-500">Loading report details...</div>;
      ...
      <div className="report">
        <div className="report-heading">
          <div className="report-heading__top">
            ...
            <span className="report-heading__date">{reportDetails.published_date || reportDetails.date}</span>
            <span className="tag">
              <span>{reportDetails.report_type || reportDetails.tag}</span>
            </span>
          </div>
          <div className="report-heading__desc">
            <p className="report-heading__sector">Sector: {reportDetails.sector}</p>
            {reportDetails.file_url && reportDetails.file_url !== '#' && (
              <a className="download-cta" href={reportDetails.file_url} target="_blank" rel="noreferrer">
          ...
          <article className="report-card" style={{ width: '100%', maxWidth: 'none' }}>
            ...
            <div className="report-card__body">
              <div className="report-card__paragraphs">
                <p>
                  {reportDetails.description || 'This audit report presents findings from our field investigations, compliance checks, and transaction testing. We examined administrative systems, procurement records, and project schedules to verify whether expenditures align with budgetary approvals.'}
                </p>
              </div>
