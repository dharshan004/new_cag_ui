...
export default function WhoWeAre() {
  ...
  const [reportsCount, setReportsCount] = useState('700+');
  ...
  useEffect(() => {
    ...
    // Fetch reports count dynamically
    fetch('/api/home')
      .then(res => res.json())
      .then(data => {
        if (data.stats && Array.isArray(data.stats)) {
          const repStat = data.stats.find((s: any) => s.label === 'Reports Tabled');
          if (repStat) setReportsCount(repStat.value);
        }
      })
      .catch(err => console.warn('Home stats live fetch skipped/failed, using fallback:', err));
      ...
      <section className="stats" data-node-id="356:17163" aria-label="Key statistics">
        <div className="stats__inner" data-node-id="356:17165">
          ...
          <div className="stat" data-node-id="356:17176">
            ...
            <p className="stat__number" data-node-id="356:17182">{reportsCount}</p>
