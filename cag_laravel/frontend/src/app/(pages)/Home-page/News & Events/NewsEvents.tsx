...
export default function NewsEvents() {
  ...
  useEffect(() => {
    const fallbackNews = dataManager.getNews();
    setTrendingNews(fallbackNews.filter(n => n.type === 'trending'));
    setFeaturedNews(fallbackNews.find(n => n.type === 'featured') || null);
      ...
      .then(data => {
        if (data.news && Array.isArray(data.news) && data.news.length > 0) {
          const formattedNews = data.news.map((n: any) => ({
            ...
            title_hi: n.title_hi,
            desc: n.content_en || '',
            desc_hi: n.content_hi || '',
            date: new Date(n.publish_date).toLocaleDateString('en-IN'),
            type: n.news_type === 'featured' ? 'featured' : 'trending',
            image: n.image_url || '/assets/cc8a1a5614f48c98f397dcafcf38e8f22843dc2a.png',
            tag: n.tag || 'Latest'
          }));
          setTrendingNews(formattedNews.filter((n: any) => n.type === 'trending'));
          setFeaturedNews(formattedNews.find((n: any) => n.type === 'featured') || formattedNews[0]);
        }
      })
      .catch(err => console.warn('News live fetch skipped/failed, using fallback news:', err));
          ...
          <div className="trending-news">
            ...
            {trendingNews.map((news) => {
              const details = isHindi ? {
                title: (news as any).title_hi || HINDI_NEWS_TRANSLATIONS[news.id]?.title || news.title,
                desc: (news as any).desc_hi || HINDI_NEWS_TRANSLATIONS[news.id]?.desc || news.desc
              } : {
                title: news.title,
                desc: news.desc
              };
          ...
          {featuredNews && (
            <Link href="/Reports/rep-3" className="featured-news cursor-pointer block hover:scale-[1.01] transition-transform">
              <img 
                ...
                alt={isHindi ? ((featuredNews as any).title_hi || HINDI_NEWS_TRANSLATIONS['news-featured']?.title || featuredNews.title) : featuredNews.title} 
              ...
              <div className="featured-news__text">
                ...
                <h3 className="featured-news__headline">
                  {isHindi ? ((featuredNews as any).title_hi || HINDI_NEWS_TRANSLATIONS['news-featured']?.title || featuredNews.title) : featuredNews.title}
                </h3>
              </div>
            </Link>
          )}
