...
export default function Banner() {
  ...
  // Fetch live banners from the database
  useEffect(() => {
    fetch('/api/home')
      .then(res => res.json())
      .then(data => {
        if (data.banners && Array.isArray(data.banners) && data.banners.length > 0) {
          const loadedSlides = data.banners.map((b: any) => ({
            image: b.image_url,
            engLine1: b.title_en || 'CAG',
            engLine2: b.subtitle_en || '',
            engLine3: '',
            hinLine1: b.title_hi || b.title_en || 'सीएजी',
            hinLine2: b.subtitle_hi || b.subtitle_en || '',
            hinLine3: '',
            engSub: b.subtitle_en || '',
            hinSub: b.subtitle_hi || b.subtitle_en || ''
          }));
          setSlides(loadedSlides);
        }
      })
      .catch(err => console.warn('Banner live fetch skipped/failed, using fallback slides:', err));
  }, []);
