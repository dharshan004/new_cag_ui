...
export default function CentralOfficesPage() {
  ...
  useEffect(() => {
    api.getPresence()
      .then((data) => {
        if (data) {
          // Filter only Central Audit Offices
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      <div className="border-b border-[#e6e6e6] pb-4">
        <h2 className="text-3xl font-extrabold text-[#2a2a2a] tracking-tight">Central Audit Offices</h2>
      </div>

      {loading ? (
        <div className="text-center py-10 text-[#0a3d30] font-medium">Loading Central Audit Offices...</div>
      ) : offices.length === 0 ? (
        <div className="text-center py-10 text-zinc-500">No Central Audit Offices found.</div>
      ) : (
