...
export default function Banner() {
  ...
  useEffect(() => {
    const loadBanners = () => {
      const b = dataManager.getBanners();
      setCustomBanners(b);
    };
    ...
    window.addEventListener('bannersChange', loadBanners);

    return () => {
      window.removeEventListener('languageChange', handleLangChange);
      window.removeEventListener('bannersChange', loadBanners);
    };
  }, []);
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setSlideIndex((prev) => (prev + 1) % (activeSlides.length || 1));
    }, 4000);
  ...
  const activeSlide = activeSlides[slideIndex] || activeSlides[0] || SLIDES[0];
      ...
      <section className="hero" data-node-id="356:17253" aria-label="Hero banner" style={{ position: 'relative' }}>
        ...
        {quickLinksOpen && (
          ...
          >
            ...
            <Link href="/Reports/accounts" onClick={() => setQuickLinksOpen(false)} className="quick-links-popover__item quick-links-popover__item--featured hover:bg-zinc-100 transition-colors" data-node-id="177:18405">
            ...
            <Link href="/Resources" onClick={() => setQuickLinksOpen(false)} className="quick-links-popover__item hover:bg-zinc-50 transition-colors" data-node-id="177:18411">
            ...
            <Link href="/Resources/Circulars" onClick={() => setQuickLinksOpen(false)} className="quick-links-popover__item hover:bg-zinc-50 transition-colors" data-node-id="177:18413">
